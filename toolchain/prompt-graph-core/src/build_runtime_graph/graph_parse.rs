...
use sqlparser::ast::{Join, JoinOperator, TableFactor};
use crate::reactivity::reactive_sql;
use crate::reactivity::reactive_sql::SQLType;
...
pub fn query_path_from_query_string(q: &String) -> anyhow::Result<Vec<Vec<String>>> {
    let dependent_on = reactive_sql::parse_tables_and_columns(&q)?;
    let mut paths = vec![];
