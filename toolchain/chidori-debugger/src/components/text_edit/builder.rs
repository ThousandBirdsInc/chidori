...
pub struct TextEdit<'t> {
    ...
    line_number_color: Option<Color32>,
}
...
impl<'t> TextEdit<'t> {
    ...
    /// Build a [`TextEdit`] focused on vim-enabled code editing.
    /// By default it comes with:
    /// - monospaced font
    /// - focus lock (tab will insert a tab character instead of moving focus)
    /// - vim mode enabled
    /// - line numbers shown
    pub fn vim_code_editor(self) -> Self {
    ...
    /// Enable Vim mode for the TextEdit.
    #[inline]
    pub fn vim_mode_enabled(mut self, enabled: bool) -> Self {
        self.vim_mode_enabled = enabled;
        self
    }

    /// Show line numbers in the TextEdit.
    #[inline]
    pub fn show_line_numbers(mut self, enabled: bool) -> Self {
        self.show_line_numbers = enabled;
        self
    }

    /// Set the color for line numbers in the TextEdit.
    #[inline]
    pub fn line_number_color(mut self, color: Option<Color32>) -> Self {
...
impl<'t> TextEdit<'t> {
    ...
    fn show_content(self, ui: &mut Ui) -> TextEditOutput {
        ...
        // Load state first so we can use it for calculations
        let temp_id = id.unwrap_or_else(|| {
            if let Some(id_source) = id_source {
                ui.make_persistent_id(id_source)
            } else {
                Id::new("text_edit") // temporary id for calculation
            }
        });
        ...
        let available_width = (ui.available_width() - margin.sum().x - state.line_number_width).at_least(MIN_WIDTH);
        ...
        let desired_inner_size = vec2(desired_width + state.line_number_width, galley.size().y.max(desired_height));
        ...
        // Use the final id - either the provided one, persistent id, or auto_id
        ...
        let allow_drag_to_select =
            ui.input(|i| !i.has_touch_screen()) || ui.memory(|mem| mem.has_focus(final_id));
        ...
        let mut response = ui.interact(outer_rect, final_id, sense);
        ...
        if interactive && ui.memory(|mem| mem.has_focus(final_id)) {
            ui.memory_mut(|mem| mem.set_focus_lock_filter(final_id, event_filter));
        ...
        // Adjust galley position to account for line numbers
        if state.show_line_numbers && multiline {
            galley_pos.x += state.line_number_width;
        }
        
        let align_offset = rect.left() + state.line_number_width - galley_pos.x;

        // Visual clipping for singleline text editor with text larger than width
        if clip_text && align_offset == 0.0 {
            let cursor_pos = match (cursor_range, ui.memory(|mem| mem.has_focus(final_id))) {
        ...
        if ui.is_rect_visible(rect) {
            ...
            if ui.memory(|mem| mem.has_focus(final_id)) {
        ...
        state.clone().store(ui.ctx(), final_id);
...
fn events(
    ...
    if vim_mode_enabled && !state.vim_state.is_normal_mode() && !state.vim_state.is_insert_mode() {
        ...
        if vim_mode_enabled {
            let vim_handled = match event {
                Event::Key {
                    ...
                    if state.vim_state.is_insert_mode() {
                        state.vim_state.enter_mode(VimMode::Normal);
                        true
                    } else {
                        false
                ...
                Event::Text(text_input) if state.vim_state.is_normal_mode() => {
                    if let Some(ch) = text_input.chars().next() {
                        if VimMotions::handle_normal_mode_key(
                            ...
                            // Re-layout the galley after vim commands that modify text
