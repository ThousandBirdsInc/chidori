...
- **Supervised multi-agent processes** — spawn agent modules as concurrent,
  ...
  passing, and runtime-owned restart policies — including restart-with-history,
  which replays an actor's completed work and retries only the failing call.
...
| [Actors & supervision](./docs/actors.md) | Spawned agent processes, message passing, restart strategies |
