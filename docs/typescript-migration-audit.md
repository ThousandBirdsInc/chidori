...
- `src/runtime/typescript/tools.rs` now uses the snapshot-capable
  ...
  path. Native `chidori.http` covers policy denial, and native
  `chidori.input` covers pause-mode pending input. Plain-text `chidori.prompt`
