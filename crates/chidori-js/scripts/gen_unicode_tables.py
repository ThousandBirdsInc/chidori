...
def build_case_folding():
    """Simple case folding: the C+S entries of CaseFolding.txt, as a sorted
    (code point, folded code point) map. This is the scf() the RegExp
    Canonicalize operation uses in unicode mode."""
    out = []
    with open(os.path.join(UCD, "CaseFolding.txt"), encoding="utf-8") as f:
        for line in f:
            ...
            if len(fields) < 3 or fields[1] not in ("C", "S"):
                continue
            out.append((int(fields[0], 16), int(fields[2], 16)))
    out.sort()
    return out


def emit(slices, keys, case_folds):
    lines = []
    ...
    lines.append("//")
    lines.append("// The generator emits the complete UCD property set; builtins consume only a")
    lines.append("// subset, so unused tables are expected (dead_code allow below), and keeping")
    lines.append("// the full set beats special-casing the generator per consumer.")
    ...
    lines.append("/// Simple case folding (CaseFolding.txt C+S entries), sorted by code")
    lines.append("/// point: the scf() behind RegExp Canonicalize in unicode mode.")
    body = ", ".join(f"(0x{a:X},0x{b:X})" for a, b in case_folds)
    lines.append(f"static SIMPLE_CASE_FOLD: &[(u32, u32)] = &[{body}];")
    lines.append("")
    lines.append(r'''/// Unicode simple case fold of `cp` (identity when unmapped).
pub fn simple_case_fold(cp: u32) -> u32 {
    match SIMPLE_CASE_FOLD.binary_search_by(|(k, _)| k.cmp(&cp)) {
        Ok(i) => SIMPLE_CASE_FOLD[i].1,
        Err(_) => cp,
    }
}
''')
...
def main():
    slices, keys = build()
    emit(slices, keys, build_case_folding())
