...
fn functions_and_closures() {
    ...
    assert_eq!(
        run("const fib = n => n < 2 ? n : fib(n-1) + fib(n-2); fib(10)"),
        "55"
    );
}
...
fn control_flow() {
    assert_eq!(
        run("let s = 0; for (let i = 1; i <= 10; i++) s += i; s"),
        "55"
    );
    assert_eq!(
        run("let s = 0; let i = 0; while (i < 5) { s += i; i++; } s"),
        "10"
    );
    assert_eq!(
        run("let x = 3; let r; if (x > 2) r = 'big'; else r = 'small'; r"),
        "big"
    );
...
fn objects_and_arrays() {
    ...
    assert_eq!(
        run("const a = [1,2,3]; a.map(x => x * 2).join(',')"),
        "2,4,6"
    );
    assert_eq!(
        run("[1,2,3,4,5].filter(x => x % 2 === 0).reduce((a,b) => a+b, 0)"),
        "6"
    );
    assert_eq!(run("const { x, y } = { x: 10, y: 20 }; x + y"), "30");
    assert_eq!(
        run("const [a, ...rest] = [1,2,3,4]; rest.join('-')"),
        "2-3-4"
    );
    assert_eq!(
...
fn classes() {
    ...
    assert_eq!(
        run("class A { greet() { return 'hi'; } } class B extends A { } new B().greet()"),
        "hi"
    );
...
fn strings_and_templates() {
    assert_eq!(
        run("const n = 42; `the answer is ${n}`"),
        "the answer is 42"
    );
    assert_eq!(
        run("'hello world'.split(' ').map(s => s.toUpperCase()).join('_')"),
        "HELLO_WORLD"
    );
...
fn exceptions() {
    ...
    assert_eq!(
        run("let r = ''; try { throw 1; } catch (e) { r = 'caught'; } finally { r += '+fin'; } r"),
        "caught+fin"
    );
    assert_eq!(
...
fn generator_return_runs_finally() {
    ...
    assert_eq!(
        run(
            "let log=''; function* g(){ try { yield 1; yield 2; } finally { log+='F'; } } \
...
fn higher_order_and_spread() {
    ...
    assert_eq!(
        run("const o = {...{a:1}, ...{b:2}}; JSON.stringify(o)"),
        r#"{"a":1,"b":2}"#
    );
