...
fn generators() {
    assert_eq!(
        console("function* g(){ yield 1; yield 2; yield 3; } console.log([...g()].join(','))"),
        "1,2,3"
    );
...
fn promises() {
    assert_eq!(
        console("Promise.resolve(42).then(v => console.log(v))"),
        "42"
    );
    assert_eq!(
        console("Promise.reject('boom').catch(e => console.log('caught', e))"),
        "caught boom"
    );
    assert_eq!(console("Promise.all([Promise.resolve(1), Promise.resolve(2), 3]).then(a => console.log(a.join(',')))"), "1,2,3");
    assert_eq!(
        console("Promise.resolve(1).then(x => x + 1).then(x => x * 10).then(x => console.log(x))"),
        "20"
    );
}
